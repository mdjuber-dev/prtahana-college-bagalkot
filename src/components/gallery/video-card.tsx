...
export default function VideoCard({ video }: VideoCardProps) {
  return (
    <div className="relative group rounded-2xl overflow-hidden card-shadow cursor-pointer" role="button" tabIndex={0} aria-label={`Play video: ${video.title}`}>
      <img src={video.src} alt={video.alt} className="w-full aspect-video object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" decoding="async" />
